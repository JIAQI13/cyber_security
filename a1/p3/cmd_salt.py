...
# encrypt
cmd = 'openssl enc -e -des-{} -in {} -out {}{}.salted.enc -pass file:pw'
for mode in modes:
    for file in files:
...
for mode in modes:
    for file in files:
        with open('{}{}.error.salted.enc'.format(file,mode), 'wb') as error_file:
            with open('{}{}.salted.enc'.format(file, mode), 'rb') as orig_file:
...
# decrypt error
cmd = 'openssl enc -d -des-{} -in {}{}.error.salted.enc -out {}{}.error.salted.dec -pass file:pw'
for mode in modes:
    for file in files:
...
for file in files:
    with open(file) as orig_file:
        ...
        for mode in modes:
            print('------------------')
            print('diff {} {}{}.error.salted.dec'.format(file, file, mode))
            with open('{}{}.error.salted.dec'.format(file, mode),errors='replace') as error_file:
